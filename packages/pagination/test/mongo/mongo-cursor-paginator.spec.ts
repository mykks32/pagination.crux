...
describe('paginate (Mongo cursor paginator)', () => {
  ...
  it('returns the first page with hasNextPage=true and hasPreviousPage=false', async () => {
    ...
    expect(result.edges.map((e) => e.node._id)).toEqual(['1', '2']);
  ...
  it('pages forward using the previous page endCursor', async () => {
    ...
    expect(secondPage.edges.map((e) => e.node._id)).toEqual(['3', '4']);
  ...
  it('reaches the last page with hasNextPage=false', async () => {
    ...
    expect(secondPage.edges.map((e) => e.node._id)).toEqual(['5']);
    expect(secondPage.pageInfo.hasNextPage).toBe(false);
  });

  it('pages backward from a "before" cursor and returns nodes in ascending order', async () => {
    const model = createModelMock(docs);
    const result = await paginate(model, { args: { last: 2, before: encodeCursor([docs[4].createdAt, docs[4]._id]) }, sort });

    expect(result.edges.map((e) => e.node._id)).toEqual(['3', '4']);
    expect(result.pageInfo).toMatchObject({ hasPreviousPage: true, hasNextPage: true });
  });

  it('appends _id as an implicit tiebreaker so the emitted cursor encodes both fields', async () => {
    const model = createModelMock(docs);
    const result = await paginate(model, { args: { first: 1 }, sort });
    expect(decodeCursor(result.edges[0].cursor).values).toEqual([docs[0].createdAt, docs[0]._id]);
  });
  ...
  it('clamps limits above MAX_PAGE_SIZE', async () => {
    ...
    expect(result.edges).toHaveLength(docs.length);
  });
});
